...
vBasicWEBServer( void *pvParameters )
{
	...
	netif_set_hostname(web_info->ci40_if, "ci40_demo");
	dhcp_start( web_info->ci40_if );

	while ( web_info->ci40_if->ip_addr.addr == 0L ) {
		u32 mscnt = 0;
	...
	{
		static u8 ip[4];
		ip[0] = web_info->ci40_if->ip_addr.addr & 0xFF;
		ip[1] = (web_info->ci40_if->ip_addr.addr >> 8) & 0xFF;
		ip[2] = (web_info->ci40_if->ip_addr.addr >> 16) & 0xFF;
		ip[3] = (web_info->ci40_if->ip_addr.addr >> 24) & 0xFF;
		fprintf(web_info->dbgFile,"IP:%d.%d.%d.%d\r\n",ip[0],ip[1],ip[2],ip[3]);
	}
